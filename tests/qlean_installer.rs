...
async fn installer_runs_inside_an_isolated_vm() -> Result<()> {
    ...
    with_machine(&image, &config, |vm| {
        Box::pin(async move {
            ...
            run_checked(
                vm,
                "apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends bash coreutils curl findutils python3 sudo tar util-linux",
            )
            .await?;
            run_checked(
                vm,
            ...
            run_checked(
                vm,
                &format!(
                    "nohup python3 -m http.server 18080 --bind 127.0.0.1 --directory {root}/release >/tmp/scorpiofs-qlean-http.log 2>&1 </dev/null & echo $! >/tmp/scorpiofs-qlean-http.pid"
                ),
            )
            .await?;
            run_checked(
                vm,
                &format!(
                    "curl --fail --retry 20 --retry-delay 1 http://127.0.0.1:18080/{VERSION}/scorpiofs-{VERSION}-{TARGET}.tar.gz.sha256 >/dev/null"
                ),
            )
            .await?;
            run_checked(
            ...
            .await?;
            run_checked(vm, "kill \"$(cat /tmp/scorpiofs-qlean-http.pid)\"").await?;
            Ok(())
        })
    })
    .await
